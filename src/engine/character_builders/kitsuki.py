"""Kitsuki Magistrate school builder.

Builds Kitsuki Magistrate characters from earned XP. School ring is Water
(starts at 3, max 6), other rings start at 2. School knacks
(Discern Honor, Iaijutsu, Presence) start at rank 1 for free.
Investigation is hardcoded at rank 5 (cost absorbed by non-combat %).
...
KITSUKI_KNACK_NAMES: list[str] = ["discern_honor", "iaijutsu", "presence"]

# Ring priority: Water > Void > Air > Earth > Fire
KITSUKI_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
    "discern_honor", "iaijutsu", "presence",  # 1->2
    "attack", "parry",
    "discern_honor", "iaijutsu", "presence",  # 2->3
    "water", "void", "air", "earth", "fire",
    "attack", "parry",
    "discern_honor", "iaijutsu", "presence",  # 3->4
    "water", "void", "air", "earth", "fire",
    "attack", "parry",
    "discern_honor", "iaijutsu", "presence",  # 4->5
    "water", "void", "air", "earth", "fire",
    "attack",
]


def compute_kitsuki_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for a Kitsuki Magistrate.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in KITSUKI_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if water_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in KITSUKI_KNACK_NAMES):
            ring_values["water"] = min(ring_values["water"] + 1, 6)
            water_boosted = True
            water_cost_discount = 5

    for entry in KITSUKI_COMBAT_PROGRESSION:
...
def build_kitsuki_from_xp(
    name: str = "Kitsuki",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build a Kitsuki Magistrate character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in KITSUKI_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in KITSUKI_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in KITSUKI_KNACK_NAMES):
    ...
    knack_display_names: dict[str, tuple[str, RingName]] = {
        "discern_honor": ("Discern Honor", RingName.WATER),
        "iaijutsu": ("Iaijutsu", RingName.FIRE),
        "presence": ("Presence", RingName.EARTH),
    }
    ...
    skills: list[Skill] = [
        ...
        Skill(name="Investigation", rank=5, skill_type=SkillType.BASIC, ring=RingName.WATER),
    ]
    ...
    return Character(
        ...
        school_knacks=["Discern Honor", "Iaijutsu", "Presence"],
        xp_total=total_xp,
        xp_spent=xp_spent,
    )
