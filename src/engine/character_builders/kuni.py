"""Kuni Witch Hunter school builder.

Builds Kuni Witch Hunter characters from earned XP. School ring is Earth
(starts at 3, max 6), other rings start at 2. School knacks
(Detect Taint, Iaijutsu, Presence) start at rank 1 for free.
...
KUNI_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
    "detect_taint", "iaijutsu", "presence",  # 1->2
    "attack", "parry",
    "detect_taint", "iaijutsu", "presence",  # 2->3
    "earth", "void", "water", "air", "fire",
    "attack", "parry",
    "detect_taint", "iaijutsu", "presence",  # 3->4
    "earth", "void", "water", "air", "fire",
    "attack", "parry",
    "detect_taint", "iaijutsu", "presence",  # 4->5
    "earth", "void", "water", "air", "fire",
    "attack",
]


def compute_kuni_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for a Kuni Witch Hunter.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in KUNI_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if earth_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in KUNI_KNACK_NAMES):
            ring_values["earth"] = min(ring_values["earth"] + 1, 6)
            earth_boosted = True
            earth_cost_discount = 5

    for entry in KUNI_COMBAT_PROGRESSION:
...
def build_kuni_from_xp(
    name: str = "Kuni Witch Hunter",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build a Kuni Witch Hunter character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in KUNI_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in KUNI_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in KUNI_KNACK_NAMES):
    ...
    knack_display_names: dict[str, tuple[str, RingName]] = {
        "detect_taint": ("Detect Taint", RingName.EARTH),
    ...
    skills: list[Skill] = [
        ...
        Skill(name="Investigation", rank=5, skill_type=SkillType.BASIC, ring=RingName.EARTH),
    ]
