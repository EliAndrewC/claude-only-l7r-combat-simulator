"""Bayushi Bushi school builder.

Builds Bayushi Bushi characters from earned XP. School ring is Fire
(starts at 3, max 6), other rings start at 2. School knacks
(Double Attack, Feint, Iaijutsu) start at rank 1 for free.
...
BAYUSHI_KNACK_NAMES: list[str] = ["double_attack", "feint", "iaijutsu"]

# Ring priority: Fire > Void > Air > Water > Earth
BAYUSHI_COMBAT_PROGRESSION: list[str] = [
    # Skills/knacks to 3 (before non-Fire rings reach 3)
    "parry",
    "attack", "parry",
    "double_attack", "feint", "iaijutsu",  # 1->2
    "attack", "parry",
    "double_attack", "feint", "iaijutsu",  # 2->3

    # Rings first raise (Fire 3->4, Void 2->3, Air 2->3, Water 2->3, Earth 2->3)
    "fire", "void", "air", "water", "earth",

    # Skills/knacks to 4 (before non-Fire rings reach 4)
    "attack", "parry",
    "double_attack", "feint", "iaijutsu",  # 3->4
    ...
    "attack", "parry",
    "double_attack", "feint", "iaijutsu",  # 4->5
...
def compute_bayushi_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for a Bayushi Bushi.
    ...
    ring_values: dict[str, int] = {
        "fire": 3, "air": 2, "earth": 2, "water": 2, "void": 2,
    }

    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in BAYUSHI_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if fire_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in BAYUSHI_KNACK_NAMES):
            ring_values["fire"] = min(ring_values["fire"] + 1, 6)
            fire_boosted = True
            fire_cost_discount = 5

    for entry in BAYUSHI_COMBAT_PROGRESSION:
...
def build_bayushi_from_xp(
    name: str = "Bayushi",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build a Bayushi Bushi character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in BAYUSHI_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in BAYUSHI_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in BAYUSHI_KNACK_NAMES):
    ...
    skills = [
        Skill(
            name="Attack", rank=stats.attack,
            skill_type=SkillType.ADVANCED, ring=RingName.FIRE,
        ),
        Skill(
            name="Parry", rank=stats.parry,
