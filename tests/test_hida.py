"""Tests for the Hida Bushi school builder."""

from src.engine.character_builders.hida import (
    build_hida_from_xp,
    compute_hida_stats_from_xp,
)
...
class TestStartingValues:
    def test_starting_rings(self) -> None:
        """Water=3, others=2 at 0 XP / 100% non-combat."""
        stats = compute_hida_stats_from_xp(earned_xp=0, non_combat_pct=1.0)
    ...
    def test_school_ring_is_water(self) -> None:
        """School ring is always Water."""
        char = build_hida_from_xp(earned_xp=0, non_combat_pct=1.0)
...
class TestProgression:
    def test_zero_xp_progression(self) -> None:
        ...
        stats = compute_hida_stats_from_xp(earned_xp=0, non_combat_pct=0.0)
        assert stats.parry >= 1
        assert stats.attack >= 1

    def test_ring_priority_water_first(self) -> None:
        """Water ring advances before other rings."""
        stats = compute_hida_stats_from_xp(earned_xp=150, non_combat_pct=0.0)
        # Water starts at 3, should be raised first
        assert stats.ring_values["water"] >= 4
        # Water should be >= all other rings
        assert stats.ring_values["water"] >= stats.ring_values["air"]

    def test_knacks_before_rings(self) -> None:
        """Knacks reach 2 before Air reaches 3."""
        stats = compute_hida_stats_from_xp(earned_xp=50, non_combat_pct=0.0)
        for knack in ("counterattack", "iaijutsu", "lunge"):
            assert stats.knack_ranks[knack] >= 2

    def test_4th_dan_water_boost(self) -> None:
        """When all knacks reach 4, Water gets +1 for free."""
        stats_high = compute_hida_stats_from_xp(earned_xp=350, non_combat_pct=0.0)
        for knack in ("counterattack", "iaijutsu", "lunge"):
            assert stats_high.knack_ranks[knack] >= 4
        # Water should be boosted
        assert stats_high.ring_values["water"] >= 5

    def test_4th_dan_water_cost_discount(self) -> None:
        """Water raise after 4th Dan costs 5 less effectively."""
        stats = compute_hida_stats_from_xp(earned_xp=350, non_combat_pct=0.0)
        assert stats.ring_values["water"] >= 5

    def test_parry_constraint_holds(self) -> None:
        """Parry never exceeds attack + 1."""
        for xp in (0, 50, 100, 150, 200, 250, 300, 350):
            stats = compute_hida_stats_from_xp(earned_xp=xp, non_combat_pct=0.0)
...
class TestCharacterConstruction:
    def test_stats_consistency(self) -> None:
        """compute_hida_stats_from_xp matches build_hida_from_xp."""
        for xp in (0, 100, 200, 350):
            stats = compute_hida_stats_from_xp(earned_xp=xp, non_combat_pct=0.2)
            char = build_hida_from_xp(earned_xp=xp, non_combat_pct=0.2)
    ...
    def test_school_knacks_on_character(self) -> None:
        ...
        char = build_hida_from_xp(earned_xp=0, non_combat_pct=1.0)
        assert char.school_knacks == ["Counterattack", "Iaijutsu", "Lunge"]

    def test_school_name(self) -> None:
        """Character school is set correctly."""
        char = build_hida_from_xp(earned_xp=0)
        assert char.school == "Hida Bushi"

    def test_knack_skills_on_character(self) -> None:
        """Character has skill objects for each knack."""
        char = build_hida_from_xp(earned_xp=100, non_combat_pct=0.0)
        for name in ("Counterattack", "Iaijutsu", "Lunge"):
