"""Shosuro Actor school builder.

Builds Shosuro Actor characters from earned XP. School ring is Air
(starts at 3, max 6), other rings start at 2. School knacks
(Athletics, Discern Honor, Pontificate) start at rank 1 for free.
Acting and Sincerity are hardcoded at rank 5 (cost absorbed by non-combat %).
...
SHOSURO_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
    "athletics", "discern_honor", "pontificate",  # 1->2
    "attack", "parry",
    "athletics", "discern_honor", "pontificate",  # 2->3
    "air", "void", "water", "earth", "fire",
    "attack", "parry",
    "athletics", "discern_honor", "pontificate",  # 3->4
    "air", "void", "water", "earth", "fire",
    "attack", "parry",
    "athletics", "discern_honor", "pontificate",  # 4->5
    "air", "void", "water", "earth", "fire",
    "attack",
]


def compute_shosuro_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for a Shosuro Actor.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in SHOSURO_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if air_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in SHOSURO_KNACK_NAMES):
            ring_values["air"] = min(ring_values["air"] + 1, 6)
            air_boosted = True
            air_cost_discount = 5

    for entry in SHOSURO_COMBAT_PROGRESSION:
...
def build_shosuro_from_xp(
    name: str = "Shosuro Actor",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build a Shosuro Actor character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in SHOSURO_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in SHOSURO_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in SHOSURO_KNACK_NAMES):
    ...
    knack_display_names: dict[str, tuple[str, RingName]] = {
        ...
        "discern_honor": ("Discern Honor", RingName.AIR),
        "pontificate": ("Pontificate", RingName.AIR),
    }
    ...
    skills: list[Skill] = [
        ...
        Skill(name="Acting", rank=5, skill_type=SkillType.BASIC, ring=RingName.AIR),
        Skill(name="Sincerity", rank=5, skill_type=SkillType.BASIC, ring=RingName.AIR),
    ]
