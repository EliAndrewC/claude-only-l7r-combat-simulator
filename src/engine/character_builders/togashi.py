"""Togashi Ise Zumi school builder.

Builds Togashi Ise Zumi characters from earned XP. School ring is Void
(starts at 3, max 6), other rings start at 2. School knacks
(Athletics, Conviction, Dragon Tattoo) start at rank 1 for free.
...
TOGASHI_KNACK_NAMES: list[str] = [
    "athletics", "conviction", "dragon_tattoo",
]

# Ring priority: Void > Water > Earth > Air > Fire
TOGASHI_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
    "athletics", "conviction", "dragon_tattoo",  # 1->2
    "attack", "parry",
    "athletics", "conviction", "dragon_tattoo",  # 2->3
    "void", "water", "earth", "air", "fire",
    "attack", "parry",
    "athletics", "conviction", "dragon_tattoo",  # 3->4
    "void", "water", "earth", "air", "fire",
    "attack", "parry",
    "athletics", "conviction", "dragon_tattoo",  # 4->5
    "void", "water", "earth", "air", "fire",
    "attack",
]


def compute_togashi_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for a Togashi Ise Zumi.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in TOGASHI_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if void_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in TOGASHI_KNACK_NAMES):
            ring_values["void"] = min(ring_values["void"] + 1, 6)
            void_boosted = True
            void_cost_discount = 5

    for entry in TOGASHI_COMBAT_PROGRESSION:
...
def build_togashi_from_xp(
    name: str = "Togashi",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build a Togashi Ise Zumi character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in TOGASHI_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in TOGASHI_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in TOGASHI_KNACK_NAMES):
    ...
    knack_display_names: dict[str, tuple[str, RingName]] = {
        "athletics": ("Athletics", RingName.WATER),
        "conviction": ("Conviction", RingName.EARTH),
        "dragon_tattoo": ("Dragon Tattoo", RingName.VOID),
    }
