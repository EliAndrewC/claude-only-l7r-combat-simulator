"""Tests for the Bayushi Bushi character builder."""

from __future__ import annotations

from src.engine.character_builders.bayushi import (
    BAYUSHI_KNACK_NAMES,
    build_bayushi_from_xp,
    compute_bayushi_stats_from_xp,
)
...
class TestStartingValues:
    ...
    def test_rings_at_zero_xp(self) -> None:
        stats = compute_bayushi_stats_from_xp(earned_xp=0, non_combat_pct=1.0)
        assert stats.ring_values["fire"] == 3  # School ring
...
class TestProgression:
    ...
    def test_fire_raised_before_other_rings(self) -> None:
        stats = compute_bayushi_stats_from_xp(earned_xp=100, non_combat_pct=0.0)
        # Fire (school ring) should be raised first
        assert stats.ring_values["fire"] >= stats.ring_values["air"]
        assert stats.ring_values["fire"] >= stats.ring_values["water"]
        assert stats.ring_values["fire"] >= stats.ring_values["earth"]

    def test_knacks_advance_before_rings(self) -> None:
        stats = compute_bayushi_stats_from_xp(earned_xp=50, non_combat_pct=0.0)
    ...
    def test_parry_constrained_by_attack(self) -> None:
        for xp in [50, 100, 150, 200]:
            stats = compute_bayushi_stats_from_xp(earned_xp=xp, non_combat_pct=0.0)
    ...
    def test_4th_dan_fire_boost(self) -> None:
        """When all knacks reach 4, Fire gets a free +1."""
        # Find XP where all knacks reach 4
        for xp in range(0, 400, 10):
            stats = compute_bayushi_stats_from_xp(earned_xp=xp, non_combat_pct=0.0)
            if all(stats.knack_ranks[k] >= 4 for k in BAYUSHI_KNACK_NAMES):
                # Fire should be boosted beyond base progression
                assert stats.ring_values["fire"] >= 4
                break

    def test_fire_max_is_6(self) -> None:
        stats = compute_bayushi_stats_from_xp(earned_xp=500, non_combat_pct=0.0)
        assert stats.ring_values["fire"] <= 6
    ...
    def test_high_xp_progression(self) -> None:
        stats = compute_bayushi_stats_from_xp(earned_xp=350, non_combat_pct=0.20)
        assert stats.attack >= 3
        assert stats.parry >= 2


class TestCharacterConstruction:
    """Tests for build_bayushi_from_xp()."""

    def test_stats_match_builder(self) -> None:
        xp = 200
        stats = compute_bayushi_stats_from_xp(earned_xp=xp, non_combat_pct=0.20)
        char = build_bayushi_from_xp(earned_xp=xp, non_combat_pct=0.20)
        ring_map = {
    ...
    def test_school_name(self) -> None:
        char = build_bayushi_from_xp(earned_xp=0)
        assert char.school == "Bayushi Bushi"
    ...
    def test_knack_skills_exist(self) -> None:
        char = build_bayushi_from_xp(earned_xp=100)
