"""Otaku Bushi school builder.

Builds Otaku Bushi characters from earned XP. School ring is Fire
...
def compute_otaku_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for an Otaku Bushi.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in OTAKU_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if fire_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in OTAKU_KNACK_NAMES):
            ring_values["fire"] = min(ring_values["fire"] + 1, 6)
            fire_boosted = True
            fire_cost_discount = 5

    for entry in OTAKU_COMBAT_PROGRESSION:
...
def build_otaku_from_xp(
    name: str = "Otaku",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build an Otaku Bushi character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in OTAKU_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in OTAKU_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in OTAKU_KNACK_NAMES):
