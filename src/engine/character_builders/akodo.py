"""Akodo Bushi school builder.

Builds Akodo Bushi characters from earned XP. School ring is Water
...
AKODO_KNACK_NAMES: list[str] = ["double_attack", "feint", "iaijutsu"]

# Ring priority: Water > Void > Air > Earth > Fire
AKODO_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
...
def compute_akodo_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for an Akodo Bushi.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in AKODO_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if water_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in AKODO_KNACK_NAMES):
            ring_values["water"] = min(ring_values["water"] + 1, 6)
            water_boosted = True
            water_cost_discount = 5

    for entry in AKODO_COMBAT_PROGRESSION:
...
def build_akodo_from_xp(
    name: str = "Akodo",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build an Akodo Bushi character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in AKODO_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in AKODO_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in AKODO_KNACK_NAMES):
