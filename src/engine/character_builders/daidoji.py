"""Daidoji Yojimbo school builder.

Builds Daidoji Yojimbo characters from earned XP. School ring is Water
...
DAIDOJI_KNACK_NAMES: list[str] = ["counterattack", "double_attack", "iaijutsu"]

# Ring priority: Water > Void > Air > Earth > Fire
DAIDOJI_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
...
def compute_daidoji_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for a Daidoji Yojimbo.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in DAIDOJI_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if water_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in DAIDOJI_KNACK_NAMES):
            ring_values["water"] = min(ring_values["water"] + 1, 6)
            water_boosted = True
            water_cost_discount = 5

    for entry in DAIDOJI_COMBAT_PROGRESSION:
...
def build_daidoji_from_xp(
    name: str = "Daidoji",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build a Daidoji Yojimbo character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in DAIDOJI_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in DAIDOJI_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in DAIDOJI_KNACK_NAMES):
