"""Hida Bushi school builder.

Builds Hida Bushi characters from earned XP. School ring is Water
(starts at 3, max 6), other rings start at 2. School knacks
(Counterattack, Iaijutsu, Lunge) start at rank 1 for free.
...
HIDA_KNACK_NAMES: list[str] = ["counterattack", "iaijutsu", "lunge"]

# Ring priority: Water > Void > Fire > Earth > Air
HIDA_COMBAT_PROGRESSION: list[str] = [
    # Skills/knacks to 3 (before non-Water rings reach 3)
    "parry",
    "attack", "parry",
    "counterattack", "iaijutsu", "lunge",  # 1->2
    "attack", "parry",
    "counterattack", "iaijutsu", "lunge",  # 2->3

    # Rings first raise (Water 3->4, Void 2->3, Fire 2->3, Earth 2->3, Air 2->3)
    "water", "void", "fire", "earth", "air",

    # Skills/knacks to 4 (before non-Water rings reach 4)
    "attack", "parry",
    "counterattack", "iaijutsu", "lunge",  # 3->4

    # Rings second raise (Water 4->5, Void 3->4, etc.)
    "water", "void", "fire", "earth", "air",

    # Skills/knacks to 5 (before non-Water rings reach 5)
    "attack", "parry",
    "counterattack", "iaijutsu", "lunge",  # 4->5

    # Rings third raise (Water 5->6, Void 4->5, etc.)
    "water", "void", "fire", "earth", "air",
...
def compute_hida_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for a Hida Bushi.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in HIDA_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if water_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in HIDA_KNACK_NAMES):
            ring_values["water"] = min(ring_values["water"] + 1, 6)
            water_boosted = True
            water_cost_discount = 5

    for entry in HIDA_COMBAT_PROGRESSION:
...
def build_hida_from_xp(
    name: str = "Hida",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build a Hida Bushi character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in HIDA_KNACK_NAMES}
    _budget = combat_xp
    _wb = False
    _wd = 0
    ...
    for entry in HIDA_COMBAT_PROGRESSION:
        ...
        if not _wb and all(_kr[k] >= 4 for k in HIDA_KNACK_NAMES):
