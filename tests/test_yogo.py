"""Tests for the Yogo Warden character builder."""

from __future__ import annotations

from src.engine.character_builders.yogo import (
    YOGO_KNACK_NAMES,
    build_yogo_from_xp,
    compute_yogo_stats_from_xp,
)
...
class TestStartingValues:
    ...
    def test_rings_at_zero_xp(self) -> None:
        stats = compute_yogo_stats_from_xp(earned_xp=0, non_combat_pct=1.0)
...
class TestProgression:
    ...
    def test_earth_raised_before_other_rings(self) -> None:
        stats = compute_yogo_stats_from_xp(earned_xp=100, non_combat_pct=0.0)
    ...
    def test_knacks_advance_before_rings(self) -> None:
        stats = compute_yogo_stats_from_xp(earned_xp=50, non_combat_pct=0.0)
    ...
    def test_parry_constrained_by_attack(self) -> None:
        for xp in [50, 100, 150, 200]:
            stats = compute_yogo_stats_from_xp(earned_xp=xp, non_combat_pct=0.0)
    ...
    def test_4th_dan_earth_boost(self) -> None:
        ...
        for xp in range(0, 400, 10):
            stats = compute_yogo_stats_from_xp(earned_xp=xp, non_combat_pct=0.0)
            if all(stats.knack_ranks[k] >= 4 for k in YOGO_KNACK_NAMES):
                # Earth should be boosted beyond base progression
                assert stats.ring_values["earth"] >= 4
                break

    def test_earth_max_is_6(self) -> None:
        stats = compute_yogo_stats_from_xp(earned_xp=500, non_combat_pct=0.0)
    ...
    def test_high_xp_progression(self) -> None:
        stats = compute_yogo_stats_from_xp(earned_xp=350, non_combat_pct=0.20)
        assert stats.attack >= 3
        assert stats.parry >= 2


class TestCharacterConstruction:
    """Tests for build_yogo_from_xp()."""

    def test_stats_match_builder(self) -> None:
        xp = 200
        stats = compute_yogo_stats_from_xp(earned_xp=xp, non_combat_pct=0.20)
        char = build_yogo_from_xp(earned_xp=xp, non_combat_pct=0.20)
        ring_map = {
    ...
    def test_school_name(self) -> None:
        char = build_yogo_from_xp(earned_xp=0)
        assert char.school == "Yogo Warden"

    def test_school_knacks_on_char(self) -> None:
        char = build_yogo_from_xp(earned_xp=0)
        assert char.school_knacks == ["Double Attack", "Iaijutsu", "Feint"]

    def test_knack_skills_exist(self) -> None:
        char = build_yogo_from_xp(earned_xp=100)
