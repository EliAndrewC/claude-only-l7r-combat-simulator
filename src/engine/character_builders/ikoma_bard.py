"""Ikoma Bard school builder.

Builds Ikoma Bard characters from earned XP. School ring is Water
(starts at 3, max 6), other rings start at 2. School knacks
(Discern Honor, Oppose Knowledge, Oppose Social) start at rank 1 for free.
Bragging is hardcoded at rank 5 (cost absorbed by non-combat %).
...
IKOMA_BARD_KNACK_NAMES: list[str] = [
...
IKOMA_BARD_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
    "discern_honor", "oppose_knowledge", "oppose_social",  # 1->2
    "attack", "parry",
    "discern_honor", "oppose_knowledge", "oppose_social",  # 2->3
    "water", "void", "air", "earth", "fire",
    "attack", "parry",
    "discern_honor", "oppose_knowledge", "oppose_social",  # 3->4
    "water", "void", "air", "earth", "fire",
    "attack", "parry",
    "discern_honor", "oppose_knowledge", "oppose_social",  # 4->5
    "water", "void", "air", "earth", "fire",
    "attack",
]


def compute_ikoma_bard_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for an Ikoma Bard.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in IKOMA_BARD_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if water_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in IKOMA_BARD_KNACK_NAMES):
            ring_values["water"] = min(ring_values["water"] + 1, 6)
            water_boosted = True
            water_cost_discount = 5

    for entry in IKOMA_BARD_COMBAT_PROGRESSION:
...
def build_ikoma_bard_from_xp(
    name: str = "Ikoma Bard",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build an Ikoma Bard character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in IKOMA_BARD_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in IKOMA_BARD_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in IKOMA_BARD_KNACK_NAMES):
    ...
    knack_display_names: dict[str, tuple[str, RingName]] = {
        ...
        "oppose_knowledge": ("Oppose Knowledge", RingName.AIR),
        "oppose_social": ("Oppose Social", RingName.WATER),
    }
    ...
    skills: list[Skill] = [
        ...
        Skill(name="Bragging", rank=5, skill_type=SkillType.BASIC, ring=RingName.WATER),
    ]
