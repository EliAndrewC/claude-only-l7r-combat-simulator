"""Doji Artisan school builder.

Builds Doji Artisan characters from earned XP. School ring is Water
(starts at 3, max 6), other rings start at 2. School knacks
(Counterattack, Oppose Social, Worldliness) start at rank 1 for free.
Culture is hardcoded at rank 5 (cost absorbed by non-combat %).
...
DOJI_ARTISAN_KNACK_NAMES: list[str] = [
    "counterattack", "oppose_social", "worldliness",
]

# Ring priority: Water > Void > Air > Earth > Fire
DOJI_ARTISAN_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
    "counterattack", "oppose_social", "worldliness",  # 1->2
    "attack", "parry",
    "counterattack", "oppose_social", "worldliness",  # 2->3
    "water", "void", "air", "earth", "fire",
    "attack", "parry",
    "counterattack", "oppose_social", "worldliness",  # 3->4
    "water", "void", "air", "earth", "fire",
    "attack", "parry",
    "counterattack", "oppose_social", "worldliness",  # 4->5
    "water", "void", "air", "earth", "fire",
    "attack",
]


def compute_doji_artisan_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for a Doji Artisan.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in DOJI_ARTISAN_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if water_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in DOJI_ARTISAN_KNACK_NAMES):
            ring_values["water"] = min(ring_values["water"] + 1, 6)
            water_boosted = True
            water_cost_discount = 5

    for entry in DOJI_ARTISAN_COMBAT_PROGRESSION:
...
def build_doji_artisan_from_xp(
    name: str = "Doji Artisan",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build a Doji Artisan character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in DOJI_ARTISAN_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in DOJI_ARTISAN_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in DOJI_ARTISAN_KNACK_NAMES):
    ...
    knack_display_names: dict[str, tuple[str, RingName]] = {
        "counterattack": ("Counterattack", RingName.FIRE),
        "oppose_social": ("Oppose Social", RingName.WATER),
        "worldliness": ("Worldliness", RingName.WATER),
    }
    ...
    skills: list[Skill] = [
        ...
        Skill(name="Culture", rank=5, skill_type=SkillType.BASIC, ring=RingName.AIR),
    ]
