"""Hiruma Scout school builder.

Builds Hiruma Scout characters from earned XP. School ring is Air
...
HIRUMA_KNACK_NAMES: list[str] = ["double_attack", "feint", "iaijutsu"]

# Ring priority: Air > Void > Water > Earth > Fire
HIRUMA_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
...
def compute_hiruma_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for a Hiruma Scout.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in HIRUMA_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if air_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in HIRUMA_KNACK_NAMES):
            ring_values["air"] = min(ring_values["air"] + 1, 6)
            air_boosted = True
            air_cost_discount = 5

    for entry in HIRUMA_COMBAT_PROGRESSION:
...
def build_hiruma_from_xp(
    name: str = "Hiruma",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build a Hiruma Scout character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in HIRUMA_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in HIRUMA_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in HIRUMA_KNACK_NAMES):
