"""Isawa Ishi school builder.

Builds Isawa Ishi characters from earned XP. School ring is Void
(starts at 3, max 6), other rings start at 2. School knacks
(Absorb Void, Kharmic Spin, Otherworldliness) start at rank 1 for free.
Precepts is hardcoded at rank 5 (cost absorbed by non-combat %).
...
ISAWA_ISHI_KNACK_NAMES: list[str] = [
    "absorb_void", "kharmic_spin", "otherworldliness",
]

# Ring priority: Void > Water > Air > Earth > Fire
ISAWA_ISHI_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
    "absorb_void", "kharmic_spin", "otherworldliness",  # 1->2
    "attack", "parry",
    "absorb_void", "kharmic_spin", "otherworldliness",  # 2->3
    "void", "water", "air", "earth", "fire",
    "attack", "parry",
    "absorb_void", "kharmic_spin", "otherworldliness",  # 3->4
    "void", "water", "air", "earth", "fire",
    "attack", "parry",
    "absorb_void", "kharmic_spin", "otherworldliness",  # 4->5
    "void", "water", "air", "earth", "fire",
    "attack",
]


def compute_isawa_ishi_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for an Isawa Ishi.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in ISAWA_ISHI_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if void_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in ISAWA_ISHI_KNACK_NAMES):
            ring_values["void"] = min(ring_values["void"] + 1, 6)
            void_boosted = True
            void_cost_discount = 5

    for entry in ISAWA_ISHI_COMBAT_PROGRESSION:
...
def build_isawa_ishi_from_xp(
    name: str = "Isawa Ishi",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build an Isawa Ishi character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in ISAWA_ISHI_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in ISAWA_ISHI_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in ISAWA_ISHI_KNACK_NAMES):
    ...
    knack_display_names: dict[str, tuple[str, RingName]] = {
        "absorb_void": ("Absorb Void", RingName.VOID),
        "kharmic_spin": ("Kharmic Spin", RingName.VOID),
        "otherworldliness": ("Otherworldliness", RingName.VOID),
    }
    ...
    skills: list[Skill] = [
        ...
        Skill(name="Precepts", rank=5, skill_type=SkillType.BASIC, ring=RingName.WATER),
    ]
    for knack_key, (display_name, ring) in knack_display_names.items():
        skills.append(
            Skill(
                name=display_name,
