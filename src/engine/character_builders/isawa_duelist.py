"""Isawa Duelist school builder.

Builds Isawa Duelist characters from earned XP. School ring is Water
...
ISAWA_DUELIST_KNACK_NAMES: list[str] = ["double_attack", "iaijutsu", "lunge"]

# Ring priority: Water > Void > Air > Earth > Fire
ISAWA_DUELIST_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
...
def compute_isawa_duelist_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for an Isawa Duelist.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in ISAWA_DUELIST_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if water_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in ISAWA_DUELIST_KNACK_NAMES):
            ring_values["water"] = min(ring_values["water"] + 1, 6)
            water_boosted = True
            water_cost_discount = 5

    for entry in ISAWA_DUELIST_COMBAT_PROGRESSION:
...
def build_isawa_duelist_from_xp(
    name: str = "Isawa Duelist",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build an Isawa Duelist character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in ISAWA_DUELIST_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in ISAWA_DUELIST_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in ISAWA_DUELIST_KNACK_NAMES):
