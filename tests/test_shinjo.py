...
class TestStartingValues:
    def test_starting_rings(self) -> None:
        ...
        stats = compute_shinjo_stats_from_xp(earned_xp=0, non_combat_pct=1.0)
    ...
    def test_school_ring_is_air(self) -> None:
        ...
        char = build_shinjo_from_xp(earned_xp=0, non_combat_pct=1.0)
...
class TestProgression:
    def test_zero_xp_progression(self) -> None:
        ...
        stats = compute_shinjo_stats_from_xp(earned_xp=0, non_combat_pct=0.0)
        assert stats.parry >= 1
        assert stats.attack >= 1

    def test_ring_priority_air_first(self) -> None:
        """Air ring advances before other rings."""
        stats = compute_shinjo_stats_from_xp(earned_xp=150, non_combat_pct=0.0)
    ...
    def test_knacks_before_rings(self) -> None:
        ...
        stats = compute_shinjo_stats_from_xp(earned_xp=50, non_combat_pct=0.0)
    ...
    def test_4th_dan_air_boost(self) -> None:
        ...
        stats_high = compute_shinjo_stats_from_xp(earned_xp=350, non_combat_pct=0.0)
        for knack in ("double_attack", "iaijutsu", "lunge"):
            assert stats_high.knack_ranks[knack] >= 4
        # Air should be boosted (higher than if we just had the paid raises)
    ...
    def test_4th_dan_air_cost_discount(self) -> None:
        ...
        stats = compute_shinjo_stats_from_xp(earned_xp=350, non_combat_pct=0.0)
    ...
    def test_parry_constraint_holds(self) -> None:
        ...
        for xp in (0, 50, 100, 150, 200, 250, 300, 350):
            stats = compute_shinjo_stats_from_xp(earned_xp=xp, non_combat_pct=0.0)
...
class TestCharacterConstruction:
    def test_stats_consistency(self) -> None:
        """compute_shinjo_stats_from_xp matches build_shinjo_from_xp."""
        for xp in (0, 100, 200, 350):
            stats = compute_shinjo_stats_from_xp(earned_xp=xp, non_combat_pct=0.2)
            char = build_shinjo_from_xp(earned_xp=xp, non_combat_pct=0.2)
    ...
    def test_school_name(self) -> None:
        ...
        char = build_shinjo_from_xp(earned_xp=0)
        assert char.school == "Shinjo Bushi"

    def test_knack_skills_on_character(self) -> None:
        """Character has skill objects for each knack."""
        char = build_shinjo_from_xp(earned_xp=100, non_combat_pct=0.0)
