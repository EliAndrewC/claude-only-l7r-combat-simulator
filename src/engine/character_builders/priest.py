"""Priest school builder.

Builds Priest characters from earned XP. School ring is Water
(starts at 3, max 6), other rings start at 2. School knacks
(Conviction, Otherworldliness, Pontificate) start at rank 1 for free.
...
PRIEST_KNACK_NAMES: list[str] = [
    "conviction", "otherworldliness", "pontificate",
]

# Ring priority: Water > Void > Air > Earth > Fire
PRIEST_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
    "conviction", "otherworldliness", "pontificate",  # 1->2
    "attack", "parry",
    "conviction", "otherworldliness", "pontificate",  # 2->3
    "water", "void", "air", "earth", "fire",
    "attack", "parry",
    "conviction", "otherworldliness", "pontificate",  # 3->4
    "water", "void", "air", "earth", "fire",
    "attack", "parry",
    "conviction", "otherworldliness", "pontificate",  # 4->5
    "water", "void", "air", "earth", "fire",
    "attack",
]


def compute_priest_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for a Priest.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in PRIEST_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if water_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in PRIEST_KNACK_NAMES):
            ring_values["water"] = min(ring_values["water"] + 1, 6)
            water_boosted = True
            water_cost_discount = 5

    for entry in PRIEST_COMBAT_PROGRESSION:
...
def build_priest_from_xp(
    name: str = "Priest",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build a Priest character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in PRIEST_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in PRIEST_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in PRIEST_KNACK_NAMES):
