"""Ide Diplomat school builder.

Builds Ide Diplomat characters from earned XP. School ring is Water
(starts at 3, max 6), other rings start at 2. School knacks
(Double Attack, Feint, Worldliness) start at rank 1 for free.
...
IDE_DIPLOMAT_KNACK_NAMES: list[str] = [
...
IDE_DIPLOMAT_COMBAT_PROGRESSION: list[str] = [
    "parry",
    "attack", "parry",
    "double_attack", "feint", "worldliness",
    "attack", "parry",
...
def compute_ide_diplomat_stats_from_xp(
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> ComputedStats:
    """Compute ring values, skill ranks, and knack ranks for an Ide Diplomat.
    ...
    attack = 0
    parry = 0
    knack_ranks: dict[str, int] = {k: 1 for k in IDE_DIPLOMAT_KNACK_NAMES}
    budget = combat_xp
    ...
    def _check_4th_dan() -> None:
        ...
        if water_boosted:
            return
        if all(knack_ranks[k] >= 4 for k in IDE_DIPLOMAT_KNACK_NAMES):
            ring_values["water"] = min(ring_values["water"] + 1, 6)
            water_boosted = True
            water_cost_discount = 5

    for entry in IDE_DIPLOMAT_COMBAT_PROGRESSION:
...
def build_ide_diplomat_from_xp(
    name: str = "Ide Diplomat",
    earned_xp: int = 0,
    non_combat_pct: float = 0.20,
) -> Character:
    """Build an Ide Diplomat character by spending XP.
    ...
    _atk = 0
    _par = 0
    _kr: dict[str, int] = {k: 1 for k in IDE_DIPLOMAT_KNACK_NAMES}
    _budget = combat_xp
    _fb = False
    _fd = 0
    ...
    for entry in IDE_DIPLOMAT_COMBAT_PROGRESSION:
        ...
        if not _fb and all(_kr[k] >= 4 for k in IDE_DIPLOMAT_KNACK_NAMES):
    ...
    knack_display_names: dict[str, tuple[str, SkillType, RingName]] = {
        "double_attack": ("Double Attack", SkillType.ADVANCED, RingName.FIRE),
        "feint": ("Feint", SkillType.ADVANCED, RingName.FIRE),
        "worldliness": ("Worldliness", SkillType.BASIC, RingName.WATER),
    }
    ...
    skills: list[Skill] = [
        ...
        Skill(name="Tact", rank=5, skill_type=SkillType.BASIC, ring=RingName.WATER),
    ]
    for knack_key, (display_name, skill_type, ring) in knack_display_names.items():
        skills.append(
            Skill(
                name=display_name,
